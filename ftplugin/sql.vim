...
function! s:RunQueryCallback(timer, platform, server, database, job_id, data, event) " {{{1
...
function! s:AlignColumns() " {{{1
    if exists(':EasyAlign') && sql#settings#doAlign()
        normal! gg
        ...
        while startRow > 0
            ...
            if timeEstimate <= sql#settings#alignTimeLimit()
